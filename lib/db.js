...
export async function initListingsTable() {
  ...
  const { rows: cr } = await pool.query(`SELECT COUNT(*)::int AS count FROM listings WHERE type = 'cruise'`)
  if (cr[0].count === 0) {
    await pool.query(`
...
export async function createListing(type, name, color, { image_url, description, location, price, emoji, image_pos } = {}) {
  ...
  const { rows } = await pool.query(
    ...
    [type, name, color, image_url || null, description || null, location || null, price || null, emoji ?? '🚢', image_pos || null]
  )
  return rows[0]
}
...
export async function addTestimonial(name, text, image_url) {
  const pool = getPool()
  await initTestimonialsTable()
  const { rows } = await pool.query(
    'INSERT INTO testimonials (name, text, image_url) VALUES ($1, $2, $3) RETURNING *',
    [name, text, image_url || null]
  )
  return rows[0]
}

export async function updateTestimonial(id, name, text, image_url) {
  const pool = getPool()
  const { rows } = await pool.query(
    'UPDATE testimonials SET name = $1, text = $2, image_url = $3, updated_at = NOW() WHERE id = $4 RETURNING *',
    [name, text, image_url || null, id]
  )
  return rows[0]
}
...
export async function initVideoTestimonialsTable() {
  const pool = getPool()
  await pool.query(`
...
export async function getVideoTestimonials() {
  const pool = getPool()
  await initVideoTestimonialsTable()
  const { rows } = await pool.query('SELECT * FROM video_testimonials ORDER BY created_at DESC')
  return rows
}

export async function addVideoTestimonial(youtube_url) {
  const pool = getPool()
  await initVideoTestimonialsTable()
  const { rows } = await pool.query(
    'INSERT INTO video_testimonials (youtube_url) VALUES ($1) RETURNING *',
    [youtube_url]
  )
  return rows[0]
}
...
export async function deleteVideoTestimonial(id) {
  const pool = getPool()
  await pool.query('DELETE FROM video_testimonials WHERE id = $1', [id])
}
